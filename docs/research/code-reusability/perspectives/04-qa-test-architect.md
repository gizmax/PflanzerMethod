...
3. **„Gherkin po session — to je QA práce."** Ne — to je PM/Decider práce *před* session. Bez acceptance criteria jako vstupu AI improvizuje a každá varianta testuje něco jiného. Apples-to-oranges v Session 2 = arbitrary Decider's Go = re-implementace. **Q5 inverze je nejdůležitější process change celé této perspektivy.**
